...
import com.example.labor5quiz.utils.QuizController
...
class QuestionFragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var quizes = (activity as MainActivity).getMainActivityViewModel().questions
    ...
    private fun setTextForButtonsAndTextView() {
        if (questionCounter == quizController.quizList.size) {
            ...
            questionTextView.text = quizController.quizList[questionCounter].question
            button1.text = quizController.quizList[questionCounter].incorrectAnswers[0]
            button2.text = quizController.quizList[questionCounter].incorrectAnswers[1]
            if(quizController.quizList[questionCounter].type.compareTo("boolean")==0){
                button3.visibility = View.GONE
                button4.visibility = View.GONE
            }
            else {
                button3.text = quizController.quizList[questionCounter].incorrectAnswers[2]
                button4.text = quizController.quizList[questionCounter].incorrectAnswers[3]
            }
        }
    }
    ...
    private fun findGoodAnswerPosition() : Int{
        for (i in 0..quizController.quizList[questionCounter].incorrectAnswers.size-1){
            if(quizController.quizList[questionCounter].incorrectAnswers[i].compareTo(quizController.quizList[questionCounter].correctAnswer)==0){
                return i
            }
        }
        return -1
